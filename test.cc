...
TEST(NwSequence, nw_sequence_alignment_test_01) {
  ...
  EXPECT_EQ(res.string1, "GGGCGACACTCCACCATAGA-");
  EXPECT_EQ(res.string2, "GG-CGACAC-CCACCATACAT");
}
...
TEST(NwSequence, empty_test_01) {
  ...
  EXPECT_EQ(res.string1, "");
  EXPECT_EQ(res.string2, "");
}
...
TEST(RegExpr, regex_test_01) {
  ...
  bool res = DnaAlgorithm::regExprChecker(regex, text);
...
TEST(MinWindowSubstr, minimum_window_substr_test_01) {
  ...
  std::string res = DnaAlgorithm::minWindowString(str1, str2);
